...
internal class Coff
{
    ...
    private List<ImageSectionHeader> _sectionHeaders = [];
    private List<ImageSymbol> _symbols = [];
    private List<SectionAddressInfo> _sectionAddressInfos = [];
    private long _stringTableOffset;
    private byte[] _bofBytes;
    ...
    public Coff(byte[] bofBytes)
    {
        ...
        ImportPrefix = _fileHeader.Machine == ImageFileMachine.IMAGE_FILE_MACHINE_AMD64
            ? "__imp_"
            : "__imp__";
        ...
        // Ensure the number of symbols matches the expected count
        ...
        // Ensure the number of section headers matches the expected count
        ...
        BaseAddress = WriteSectionsToMemory();

        // Resolve relocations for each section
        ...
        if (RuntimeConfig.IsDebugEnabled)
        {
            Console.WriteLine($"Entry point resolved: {LookupSymbolName(entryPoint)} at address: 0x{BaseAddress.ToInt64() + entryPoint.Value:X}");
        }

        SetPermissionsForSections();
        GoDelegate goFunc = Marshal.GetDelegateForFunctionPointer<GoDelegate>((IntPtr)(BaseAddress.ToInt64() + entryPoint.Value));
        ...
        try
        {
            ...
            var thread = Lib.CreateThread(IntPtr.Zero,
                                          0,
                                          (IntPtr)(BaseAddress.ToInt64() + entryPoint.Value),
                                          IntPtr.Zero,
                                          0,
                                          IntPtr.Zero);
    ...
    private nint WriteSectionsToMemory()
    {
        ...
        foreach (var sectionHeader in _sectionHeaders)
        {
            ...
            var pagesForSection = sectionHeader.SizeOfRawData / Environment.SystemPageSize;
            if (sectionHeader.SizeOfRawData % Environment.SystemPageSize != 0)
            {
                pagesForSection++; // Round up to the next page
            }
            totalPages += (int)pagesForSection;
        }
        var totalMemoryToAllocate = totalPages * Environment.SystemPageSize;
        if (RuntimeConfig.IsDebugEnabled)
        {
        ...
        var address = Lib.VirtualAlloc(
            ...
            Lib.MEM_COMMIT_AND_RESERVE,
            Lib.PAGE_EXECUTE_READWRITE);

        if (address == IntPtr.Zero)
        {
            var error = Marshal.GetLastWin32Error();
            throw new InvalidOperationException($"Failed to allocate memory for sections. Error: {error}");
        }
        ...
        for (var i = 0; i < _sectionHeaders.Count; i++)
        {
            ...
            byte[] sectionData = _bofBytes.AsSpan(sectionOffset, (int)sectionHeader.SizeOfRawData).ToArray();

            var sectionPages = sectionHeader.SizeOfRawData / Environment.SystemPageSize;
            if (sectionHeader.SizeOfRawData % Environment.SystemPageSize != 0)
            {
                sectionPages++; // Round up to the next page
            }
            var sectionMemorySize = sectionPages * Environment.SystemPageSize;
            var sectionAddress = Lib.VirtualAlloc(IntPtr.Add(address, pagesAllocated * Environment.SystemPageSize),
                                                  (uint)sectionMemorySize,
                                                  Lib.MEM_COMMIT,
                                                  Lib.PAGE_EXECUTE_READWRITE);
            pagesAllocated += (int)sectionPages;
            if (Lib.GetLastWin32Error() != SystemErrorCodes.ERROR_SUCCESS)
            {
                var error = Lib.GetLastWin32Error();
                throw new InvalidOperationException($"Failed to allocate memory for section '{System.Text.Encoding.ASCII.GetString(sectionHeader.Name)}'. Error: {error}");
            }
            ...
            _sectionAddressInfos.Add(new SectionAddressInfo(sectionAddress, sectionHeader.Characteristics, (int)sectionMemorySize, System.Text.Encoding.ASCII.GetString(sectionHeader.Name)));
        }
        return address;
    }
    ...
    private void SetPermissionsForSections()
    {
        foreach (var section in _sectionAddressInfos)
        {
            var (x, r, w) = (section
                                .Characteristics
                                .HasFlag((uint)SectionCharacteristics.IMAGE_SCN_MEM_EXECUTE),
                            section
                                .Characteristics
                                .HasFlag((uint)SectionCharacteristics.IMAGE_SCN_MEM_READ),
                            section
                                .Characteristics
                                .HasFlag((uint)SectionCharacteristics.IMAGE_SCN_MEM_WRITE));

            uint pagePermissions = 0;
            if (x && r && w)
            {
                pagePermissions = Lib.PAGE_EXECUTE_READWRITE;
            }
            else if (x && r && !w)
            {
                pagePermissions = Lib.PAGE_EXECUTE_READ;
            }
            else if (x && !r && !w)
            {
                pagePermissions = Lib.PAGE_EXECUTE;
            }
            else if (!x && r && w)
            {
                pagePermissions = Lib.PAGE_READWRITE;
            }
            else if (!x && r && !w)
            {
                pagePermissions = Lib.PAGE_READONLY;
            }
            else if (!x && !r && !w)
            {
                pagePermissions = Lib.PAGE_NOACCESS;
            }
            ...
            Lib.VirtualProtect(section.Address, (UIntPtr)section.Size, pagePermissions, out uint oldProtect);
        }
    }

    private void ResolveRelocations(ImageSectionHeader sectionHeader)
    {
        if (sectionHeader.NumberOfRelocations == 0)
        {
            return;
        }
        ...
        for (int i = 0; i < sectionHeader.NumberOfRelocations; i++)
        {
            ...
            if (symbol.SectionNumber == ImageSectionNumber.IMAGE_SYM_UNDEFINED)
            {
                ...
                var baseSymbolName = symbolName.StartsWith(ImportPrefix) ? symbolName.Substring(ImportPrefix.Length) : symbolName;
            ...
            IntPtr relocationAddress = BaseAddress + (int)sectionHeader.PointerToRawData + (int)relocation.VirtualAddress;
    ...
    private static IntPtr ResolveExternalReference(string dllName, string functionName)
    {
        ...
        {
            var error = Marshal.GetLastWin32Error();
            throw new InvalidOperationException($"Failed to load DLL '{dllName}'. Error: {error}");
        }
        ...
        {
            var error = Marshal.GetLastWin32Error();
            throw new InvalidOperationException($"Failed to get address of function '{functionName}' in DLL '{dllName}'. Error: {error}");
        }
        if (RuntimeConfig.IsDebugEnabled)
        {
    ...
    private string LookupSymbolName(ImageSymbol symbol)
    {
        if (symbol.Name[0] == 0)
        {
            ...
            var name = _bofBytes.Skip(offset).TakeWhile(b => b != '\0').ToArray(); // Ensure we only read until the null terminator            
            return System.Text.Encoding.ASCII.GetString(name);
        }
        else
        {
            // Symbol name is a fixed-length ASCII string
